...
\item minEdge: Using the previously described distance function, the minEdge function creates a chart of edge weights, determining the minEdge between the current node and its neighbors. If the nodes not connected to the "self" node are within the set maximum distance (MAX\_CACHE\_DIST), it is added to the tables and lists the shortest distances between the cataloged nodes. A node only checks MAX_COMPARE nodes to locate a minimum edge to submit for decision for the MST class
